...
export default function AdminSection() {
    ...
    const tableData = useMemo(() => {
        ...
        if (q) {
            data = data.filter((u) => `${u.organization.name}`.toLowerCase().includes(q));
        }
    ...
    if (subscriptionsLoading) return <div className="p-4 text-center"><Loader /></div>;
    if (subscriptionsError) return <div className="p-4 text-center text-red-500"><ErrorFallback /></div>;
