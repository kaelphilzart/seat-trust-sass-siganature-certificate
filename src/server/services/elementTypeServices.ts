...
import { ICreateElementType, IUpdateElementType, IElementType } from '@/types/element-type';
...
// GET BY ID
export const getElementTypeById = async (id: string): Promise<IElementType | null> => {
    const [rows] = await pool.query('SELECT * FROM element_types WHERE id = ?', [id]);
...
export const updateElementType = async (
...
): Promise<IUpdateElementType | null> => {
  ...
  if (!fields.length) return getElementTypeById(id);
