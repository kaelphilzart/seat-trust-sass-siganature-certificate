...
import { useGetAllOrganizationUsers, createOrganizationUser, editOrganizationUser, deleteOrganizationUser } from '@/hooks/organization-user';
import { ICreateOrganizationUser, IOrganizationUser, IUpdateOrganizationUser } from '@/types/organization-user';
...
export default function OrganizationDynamic({ organizationId }: Props) {
    ...
    const { organizationOne, organizationOneLoading, organizationOneError } = useGetOneOrganization(organizationId)
    const { organizationUsers, organizationUsersLoading, organizationUsersError } = useGetAllOrganizationUsers(organizationId);
    ...
    const [addFormData, setAddFormData] = useState<ICreateOrganizationUser | null>(null);
    const [userData, setUserData] = useState<ICreateUser | null>(null);

    //edit
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editFormData, setEditFormData] = useState<IUpdateOrganizationUser | null>(null);
    const [initialData, setInitialData] = useState<IUpdateOrganizationUser | null>(null);
    ...
    const columnsUser = useMemo<ColumnDef<IOrganizationUser>[]>(() => [
        ...
        {
            header: 'Aksi', id: 'actions', size: 120, cell: ({ row }) => <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={e => { e.stopPropagation(); handleOpenEdit(row.original); }}><IconPencil className="h-4 w-4" /></Button>
                <Button size="sm" variant="destructive" onClick={e => { e.stopPropagation(); handleDelete(row.original.id ?? '', row.original.user?.email ?? ''); }}><IconTrash className="h-4 w-4" /></Button>
            </div>
        },
    ], []);
    ...
    if (organizationOneLoading || planFeatureValuesLoading || organizationUsersLoading) return <div className="p-4 text-center"><Loader /></div>;
    if (organizationOneError || planFeatureValuesError || organizationUsersError) return <div className="p-4 text-center text-red-500"><ErrorFallback /></div>;
        ...
        <div className="p-4 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                    ...
                    <DataTable columns={columns} data={tableData} filterField="feature_key" isRemovePagination={true}
                    />
                </Card>
                {/* Organization Users Table */}
                <Card>
                    <h3 className="text-xl font-semibold mb-4">Organization Users</h3>
                    <DataTable columns={columnsUser} data={tableDataUser} filterField="organization.role" isRemovePagination={true}
                    />
                </Card>
            </div>
            <Modal
