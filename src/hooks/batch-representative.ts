...
export function useGetAllBatchRepresentatives(batchId?: string) {
    const { data, error, isLoading, isValidating } = useSWR<{ data: IBatchRepresentative[] }>(
        batchId ? `${URL_BATCH_REPRESENTATIVE}/${batchId}` : URL_BATCH_REPRESENTATIVE,
...
export async function syncBatchRepresentatives(
    ...
    try {
        const res = await request<ApiResponse<any>>(URL_BATCH_REPRESENTATIVE, {
        ...
        return {
            success: false,
            message: error?.message || 'Error sync Batch Representative',
        };
    }
}
// ===========================
// DELETE BULK
// ===========================
export async function deleteBatchRepresentativesBulk(
    ids: string[]
) {
    try {
        const res = await request<ApiResponse<any>>(URL_BATCH_REPRESENTATIVE, {
        ...
        return {
            success: false,
            message: error?.message || 'Error delete Batch Representative',
        };
    }
}