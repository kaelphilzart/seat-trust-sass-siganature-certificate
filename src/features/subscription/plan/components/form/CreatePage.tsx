...
export default function CreatePage() {
  ...
  const handleFeatureChange = (id: string, value: FeatureValue) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await alert.confirmAsync(
      'Yakin ingin menambahkan plan ini?',
      handleAsync(async () => {
        await createPlan(plan, featureValues);
  ...
  if (featuresError)
    return (
      <div className="p-4 text-center text-red-500">Error loading features</div>
    );
                ...
                {f.feature_type === 'boolean' ? (
                  ...
                  <>
                    ...
                    <Input
                      ...
                      placeholder={
                        f.feature_type === 'number' ? 'Masukkan limit' : ''
                      }
                      value={String(featureValues[f.id] ?? '')}
                      onChange={(e) =>
                        handleFeatureChange(
                          f.id ?? '',
