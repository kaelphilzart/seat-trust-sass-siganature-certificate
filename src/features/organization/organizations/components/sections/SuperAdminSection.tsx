...
export default function AdminSection() {
  ...
  const [addFormData, setAddFormData] = useState<ICreateOrganization | null>(
    null
  );
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [editFormData, setEditFormData] = useState<IUpdateOrganization | null>(
    null
  );
  const [initialData, setInitialData] = useState<IUpdateOrganization | null>(
    null
  );
  ...
  const [editSubscriptionData, setEditSubscriptionData] = useState<
    Partial<ICreateSubscription>
  >({});
  const [initialSubscriptionData, setInitialSubscriptionData] =
    useState<Partial<ICreateSubscription> | null>(null);
  ...
  const handleAddSubmit = async () => {
    ...
    if (
      !subscriptionFormData?.plan_id ||
      !subscriptionFormData?.start_date ||
      !subscriptionFormData?.end_date
    )
    ...
    await alert.confirmAsync(
      'Yakin ingin menambahkan organization ini?',
      async () => {
  ...
  const handleEditSubmit = async () => {
    ...
    await alert.confirmAsync(
      ...
      async () => {
        ...
        const orgPayload: Partial<IUpdateOrganization> =
          cleanPayload(changedOrg);
        /* ================= SUBSCRIPTION ================= */
        const changedSub = initialSubscriptionData
          ? getChangedFields(editSubscriptionData, initialSubscriptionData)
          : {};

        const subPayload: Partial<IUpdateSubscription> =
          cleanPayload(changedSub);
        /* ================= CEK PERUBAHAN ================= */
        if (
  ...
  const handleOpenEdit = useCallback((org: IOrganization) => {
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        rowProps={(row) => ({
          onClick: () =>
            router.push(`organization/${encodeId(row.original.id)}`),
          className:
