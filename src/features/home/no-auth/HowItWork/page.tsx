  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute -top-35 -right-30 w-130 h-130 bg-orange-300/20 opacity-40 blur-3xl rounded-full animate-[pulse_6s_ease-in-out_infinite]" />
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-175700px] bg-[#FF7F11]/10 blur-3xl rounded-full" />
        <div className="absolute -bottom-35 left-1/2 -translate-x-1/2 w-162.5 h-162.5 bg-[#FF7F11]/20 opacity-50 blur-3xl rounded-full animate-pulse" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <h2 className="mt-5 text-4xl md:text-5xl font-bold text-gray-900">
            From creation to{' '}
            <span className="text-[#FF7F11]">verified trust</span>
          </h2>

          <p className="mt-4 text-gray-500">
            Scroll to see how Seal Trust automates certificate issuance
            end-to-end.
          </p>
        </div>
            ...
            {steps.map((step, i) => {
              ...
              return (
                ...
                >
                  {/* TEXT */}
                  <div
                    className={`${isLeft ? 'md:text-right' : 'md:order-2 md:text-left'}`}
                  >
                    <div
                      className={`text-xs font-medium transition ${isActive ? 'text-[#FF7F11]' : 'text-gray-400'}`}
                    >
                      Step {String(i + 1).padStart(2, '0')}
                    </div>

                    <h3
                      className={`mt-2 text-2xl font-semibold transition ${isActive ? 'text-gray-900' : 'text-gray-400'}`}
                    >
                      {step.title}
                    </h3>
                  ...
                  <div className={`${isLeft ? '' : 'md:order-1'} relative`}>
                    <div
                      className={`p-6 rounded-2xl border transition duration-500 bg-white shadow-sm
                    ...
                    >
                      ...
                      <div className="mt-4 space-y-2">
                        <div
                          className={`h-3 rounded w-2/3 transition ${isActive ? 'bg-orange-100' : 'bg-gray-100'}`}
                        />
                        <div
                          className={`h-3 rounded w-1/2 transition ${isActive ? 'bg-orange-100' : 'bg-gray-100'}`}
                        />
                      </div>
                    </div>
                  </div>
