...
export default function AdminSection() {
    ...
    const { organizationUsers, organizationUsersLoading, organizationUsersError } = useGetAllOrganizationUsers();
    ...
    if (organizationUsersLoading) return <div className="p-4 text-center"><Loader /></div>;
    if (organizationUsersError) return <div className="p-4 text-center text-red-500"><ErrorFallback /></div>;

    return (
        <div>
            <h2 className="mb-4 text-lg font-semibold">Daftar Users Pada Organisasi</h2>
            ...
            <DataTable columns={columns} data={tableData} filterField="email" isRemovePagination={false}
                rowProps={row => ({
                    onClick: () => router.push(`${paths.organization.base}/${encodeId(row.original.organization.id)}`),
