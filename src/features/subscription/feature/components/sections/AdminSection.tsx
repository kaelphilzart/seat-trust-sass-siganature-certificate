...
export default function AdminSection() {
  ...
  const handleAddSubmit = async () => {
    if (
      !addFormData?.feature_key ||
      !addFormData?.display_name ||
      !addFormData?.feature_type
    )
      return alert.error('Key, Type  and display name  wajib diisi!');
    await alert.confirmAsync(
      'Yakin ingin menambahkan feature ini?',
      async () => {
  ...
  const handleEditSubmit = async () => {
    if (!editFormData?.feature_key)
      return alert.error('Feature key wajib diisi!');
  ...
  const handleOpenEdit = useCallback((feature: IFeature) => {
    const mapped: IUpdateFeature = {
      id: feature.id,
  ...
  const handleDelete = useCallback(
    async (id: string, feature_key: string) => {
      await alert.confirmAsync(
        `Yakin ingin menghapus "${feature_key}"?`,
        async () => {
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={(e) => {
                  ...
                  handleDelete(
                    row.original.id ?? '',
                    row.original.feature_key?.toString() ?? ''
                  );
                }}
              >
