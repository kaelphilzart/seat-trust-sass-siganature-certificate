...
export default function AdminSection() {
  ...
  const { representatives, representativesLoading, representativesError } =
    useGetAllRepresentatives(organizationId);
  ...
  const [editFormData, setEditFormData] =
    useState<IUpdateRepresentative | null>(null);
  const [initialData, setInitialData] = useState<IUpdateRepresentative | null>(
    null
  );
  ...
  const handleEditSubmit = async () => {
    if (!editFormData?.name || !editFormData?.title)
      return alert.error('Name dan Title wajib diisi!');
    ...
    await alert.confirmAsync(
      ...
      async () => {
        ...
        const payload: Partial<IUpdateRepresentative> =
        ...
        return {
          success: !!res,
          message: res
            ? 'Representative berhasil diperbarui!'
            : 'Gagal update representative',
        };
      }
    );
  ...
  const handleOpenEdit = useCallback((representative: IRepresentative) => {
    const mapped: IUpdateRepresentative = {
