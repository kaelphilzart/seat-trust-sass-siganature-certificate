...
export function useGetAllParticipants(batchId?: string) {
    const { data, error, isLoading, isValidating } = useSWR<{ data: IParticipant[] }>(
...
export async function createBulkParticipants(data: ICreateParticipant[]) {
  try {
    const res = await request<ApiResponse<any>>(URL_PARTICIPANT, {
      method: 'POST',
      body: data,
    });
    await mutate((key) => typeof key === 'string' && key.includes('participant'));
    ...
    return {
      success: false,
      message: error?.message || 'Error sync Participant',
    };
  }
}
// ===========================
// DELETE BULK
// ===========================
export async function deleteBulkParticipants(
    ids: string[]
) {
    try {
        const res = await request<ApiResponse<any>>(URL_PARTICIPANT, {
        ...
        return {
            success: false,
            message: error?.message || 'Error delete Participant',
        };
    }
}