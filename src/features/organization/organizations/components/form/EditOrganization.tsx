...
export default function EditOrganizationForm({
    ...
    const [formOrg, setFormOrg] = useState<IUpdateOrganization>(
        organizationData ?? ({} as IUpdateOrganization)
    );

    const [formSub, setFormSub] = useState<IUpdateSubscription>(
        subscriptionData ?? ({} as IUpdateSubscription)
    );
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <Input
                    ...
                    value={formOrg.slug ?? ''}
                    onChange={(e) =>
                        handleChangeOrg('slug', e.target.value)
                    }
                    placeholder="Slug"
                />
            </div>
