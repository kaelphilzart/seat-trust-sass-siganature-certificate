...
export default function PlanDynamic({ templateId }: Props) {
  ...
  const { templateOne, templateOneLoading, templateOneError } =
    useGetOneTemplate(templateId);

  if (templateOneLoading)
    return (
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Template ini dapat digunakan untuk membuat dokumen baru dengan format
          yang sudah disiapkan.
        </p>

        {templateOne?.created_at && (
          <p className="text-xs text-muted-foreground">
            Dibuat: {new Date(templateOne.created_at).toLocaleDateString()}
          </p>
        )}
      </div>
