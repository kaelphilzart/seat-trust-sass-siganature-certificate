...
export function useGetAllOrganizationUsers(organizationId?: string) {
    const { data, error, isLoading, isValidating } = useSWR<{ data: IOrganizationUser[] }>(
        organizationId ? `${URL_ORGANIZATION_USER}/${organizationId}` : URL_ORGANIZATION_USER,
...
export async function createOrganizationUser(
    ...
    try {
        ...
        return {
            success: false,
            message: error?.message || 'Gagal membuat organization user',
        };
    }
}
...
export const deleteOrganizationUser = async (id: string) => {
    ...
    await Promise.all([
        ...
        mutate((key) => typeof key === 'string' && key.startsWith(URL_ORGANIZATION_USER)),
    ]);
    return res;
};