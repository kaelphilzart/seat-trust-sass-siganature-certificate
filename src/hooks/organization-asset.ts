...
export function useGetAllOrganizationAssets(organizationId?: string) {
  const { data, error, isLoading, isValidating } = useSWR<{
    data: IOrganizationAsset[];
  }>(
    organizationId
      ? `${URL_ORGANIZATION_ASSETS}/${organizationId}`
      : URL_ORGANIZATION_ASSETS,
...
export async function createOrganizationAsset(
  ...
  const res = await requestFile<ApiResponse<ICreateOrganizationAsset>>(
    URL_ORGANIZATION_ASSETS,
    {
      method: 'POST',
      body: formData,
  ...
  await mutate(
    (key) => typeof key === 'string' && key.startsWith(URL_ORGANIZATION_ASSETS)
  );
...
export const deleteOrganizationAsset = async (id: string) => {
  const res = await request<ApiResponse>(`${URL_ORGANIZATION_ASSETS}/${id}`, {
    method: 'DELETE',
  });
  await mutate(
    (key) => typeof key === 'string' && key.startsWith(URL_ORGANIZATION_ASSETS)
