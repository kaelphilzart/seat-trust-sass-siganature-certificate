...
// POST
export const createFeature = async (feature: ICreateFeature): Promise<ICreateFeature> => {
    const featureId = randomUUID();
    await pool.query(
        `INSERT INTO features
        (id, feature_key, display_name, description, feature_type, category)
        VALUES (?, ?, ?, ?, ?, ?)`,
        [featureId, feature.feature_key, feature.display_name,
            feature.description, feature.feature_type, feature.category
        ]
    );
...
): Promise<IUpdateFeature | null> => {
    const allowedFields = ['email', 'is_active'];
    const fields = Object.keys(data).filter((key) => allowedFields.includes(key));
    if (!fields.length) return getFeatureById(id);
    const values = fields.map((field) => (data as any)[field]);
    const setClause = fields.map((field) => `${field} = ?`).join(', ');

    await pool.query(
        `UPDATE features SET ${setClause}, updated_at = NOW() WHERE id = ?`,
        [...values, id]
    );
    await redis.del(CACHE_ALL_FEATURES);
