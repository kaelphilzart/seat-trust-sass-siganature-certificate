...
import {
  useGetAllElementType,
  deleteElementType,
  editElementType,
  createElementType,
} from '@/hooks/element-type';
import {
  IElementType,
  ICreateElementType,
  IUpdateElementType,
...
export default function ElementTypeSection() {
  ...
  const { elementTypes, elementTypesError, elementTypesLoading } =
    useGetAllElementType();

  const [search, setSearch] = useState('');
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [addFormData, setAddFormData] = useState<ICreateElementType | null>(
    null
  );
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [editFormData, setEditFormData] = useState<IUpdateElementType | null>(
    null
  );
  const [initialData, setInitialData] = useState<IUpdateElementType | null>(
    null
  );
  ...
  const handleAddSubmit = async () => {
    ...
    await alert.confirmAsync(
      'Yakin ingin menambahkan element type ini?',
      async () => {
  ...
  const handleOpenEdit = useCallback((element: IElementType) => {
