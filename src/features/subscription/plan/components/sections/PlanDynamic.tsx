...
import { useGetAllPlanFeatureValue, editPlanFeatureValue, deletePlanFeatureValue, createPlanFeatureValueByParam } from '@/hooks/plan-feature-value';
...
export default function PlanDynamic({ PlanId }: Props) {
    ...
    const { planFeatureValues, planFeatureValuesLoading, planFeatureValuesError } = useGetAllPlanFeatureValue(PlanId);
    const [search, setSearch] = useState('');
    const [isEditPlanFeatureModalOpen, setIsEditPlanFeatureModalOpen] = useState(false);
    const [editFormPlanFeature, setEditFormPlanFeature] = useState<IUpdatePlanFeatureValue | null>(null);
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [addFormData, setAddFormData] = useState<ICreatePlanFeatureValue | null>(null);

    const [isEditPlanModalOpen, setIsEditPlanModalOpen] = useState(false);
    const [editFormPlan, setEditFormPlan] = useState<IUpdatePlan | null>(null);
    const [initialData, setInitialData] = useState<IUpdatePlanFeatureValue | null>(null);
            ...
            header: 'Aksi', id: 'actions', cell: ({ row }) => {
                return (
                    <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={e => { e.stopPropagation(); handleOpenEditPlanFeature(row.original) }}>
                            <IconPencil className='h-4 w-4' />
                        </Button>
                        <Button size="sm" variant="destructive" onClick={(e) => {
                            e.stopPropagation();
                            handleDelete(row.original.id, row.original.feature?.display_name ?? "Feature");
                        }}>
    ...
    const handleEditPlan = async () => {
        ...
        await alert.confirmAsync(
            ...
            async () => {
                ...
                return { success: !!res, message: res ? 'Plan berhasil diperbarui!' : 'Gagal update plan' };
            }
        );
    ...
    const handleEditPlanFeature = async () => {
        if (!editFormPlanFeature?.feature?.display_name) return alert.error('Feature name wajib diisi!');
        if (!initialData) return alert.error('Data awal tidak tersedia.');
        await alert.confirmAsync(
            'Yakin ingin menyimpan perubahan feature ini?',
            async () => {
                const changedOnly = getChangedFields(editFormPlanFeature, initialData);
                const payload: Partial<IUpdatePlanFeatureValue> = cleanPayload(changedOnly);
                if (Object.keys(payload).length === 0) {
                    alert.info('Tidak ada perubahan data.');
                    return { success: false, message: 'Tidak ada perubahan' };
                }
                const res = await editPlanFeatureValue(editFormPlanFeature.id!, payload);
    ...
    const handleOpenEditPlanFeature = (pfv: IPlanFeatureValue) => {
    ...
    if (planOneLoading || planFeatureValuesLoading) return <div className="p-4 text-center"><Loader /></div>;
    if (planOneError || planFeatureValuesError) return <div className="p-4 text-center text-red-500"><ErrorFallback /></div>;
        ...
        <div className="p-4 space-y-6">
            ...
            <Card className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-300">Harga: {formatIDR(planOne?.price)}</p>
                </div>
                <div>
                    <Button size="sm" variant="outline" onClick={e => { handleOpenEditPlan(planOne!) }}>
                        <IconPencil className='h-4 w-4' />
                    </Button>
                    <Button size="sm" variant="secondary" onClick={() => setIsAddModalOpen(true)} className='ml-2'>
            ...
            <Modal open={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSubmit={handleAddSubmit} title="Tambah Fitur Value">
                <AddFeatureValueForm formData={addFormData ?? undefined} onChange={data =>
                    setAddFormData(prev => JSON.stringify(prev) !== JSON.stringify(data) ? (data as ICreatePlanFeatureValue) : prev
                    )} />
            </Modal>
            <Modal
