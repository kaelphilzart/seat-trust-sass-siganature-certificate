...
import { request } from '@/utils/helper-server';
...
export function useGetAllSubscriptions(planId?: string) {
  const { data, error, isLoading, isValidating } = useSWR<{
    ...
    planId ? `${SWR_KEYS.subscriptions()}/${planId}` : SWR_KEYS.subscriptions(),
...
export const editSubscription = async (
  ...
  await Promise.all([
    ...
    organizationId &&
      mutate(SWR_KEYS.organizationDetail(organizationId), undefined, {
        revalidate: true,
      }),
...
export const deleteSubscription = async (id: string) => {
  const res = await request<ApiResponse>(SWR_KEYS.subscriptionDetail(id), {
    method: 'DELETE',
  });
