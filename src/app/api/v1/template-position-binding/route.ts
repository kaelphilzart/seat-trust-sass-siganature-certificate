...
import { getAllTemplatePositionBindings, upsertTemplatePositionBindingsBulk, deleteTemplatePositionBindingsBulk } from '@/server/services/templatePositionBinding'
        ...
        for (const item of payloads) {
            if (!item.template_position_id || !item.batch_representative_id) {
                return NextResponse.json(
                    {
                        message:
                            'template_position_id & batch_representative_id wajib',
                    },
                    { status: 400 }
                );
            }
