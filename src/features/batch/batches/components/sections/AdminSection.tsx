...
export default function AdminSection() {
    ...
    const { Batches, BatchesError, BatchesLoading } = useGetAllBatch(organizationId)
    ...
    const columns = useMemo<ColumnDef<IBatch>[]>(() => [
        ...
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                return (
                    <span
                        className={`px-2 py-1 text-xs rounded ${colorMap[status] || 'bg-gray-100'
                            }`}
                    >
                        {status}
                    </span>
    ...
    return (
        <div>
            ...
            <DataTable columns={columns} data={tableData} filterField="name" isRemovePagination={false}
                rowProps={row => ({
                    onClick: () => router.push(`${paths.batch.base}/${encodeId(row.original.id)}`),
