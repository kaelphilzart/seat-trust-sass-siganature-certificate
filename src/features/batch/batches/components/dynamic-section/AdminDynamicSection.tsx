...
export default function AdminDynamicSection({ batchId }: Props) {
  ...
  const { participants, participantsLoading, participantsError } =
    useGetAllParticipants(batchId);
  const {
    batchRepresentatives,
    batchRepresentativesLoading,
    batchRepresentativesError,
  } = useGetAllBatchRepresentatives(batchId);
  ...
  const batchIdSafe = batchOne?.id;

  const handleDownloadParticipant = useCallback(
    async (participantId: string) => {
      if (!batchIdSafe || !participantId) return;

      setLoadingParticipant(true);

      try {
        await downloadCertificateParticipant(batchIdSafe, participantId);
      } finally {
  ...
  const columnsParticipant = useMemo<ColumnDef<IParticipant>[]>(
    ...
    [loadingParticipant, batchOne?.id, handleDownloadParticipant]
  );
  ...
  if (batchOneLoading || participantsLoading || batchRepresentativesLoading)
    return (
      <div className="p-4 text-center">
        <Loader />
      </div>
    );
  if (batchOneError || participantsError || batchRepresentativesError)
    return (
      ...
      <Card className="p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            <Badge variant="lightPrimary">{batchOne?.status ?? '-'}</Badge>
          </div>
