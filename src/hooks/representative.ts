...
import { ICreateRepresentative, IRepresentative, IUpdateRepresentative } from '@/types/representative';
...
export function useGetAllRepresentatives(organizationId?: string) {
    const { data, error, isLoading, isValidating } = useSWR<{ data: IRepresentative[] }>(
        organizationId ? `${URL_REPRESENTATIVE}/${organizationId}` : URL_REPRESENTATIVE,
...
export async function createRepresentative(data: ICreateRepresentative) {
    try {
        const res = await request<ApiResponse<IRepresentative>>(URL_REPRESENTATIVE, {
            method: 'POST',
            body: data,
        });

        await mutate((key) =>
            typeof key === 'string' && key.startsWith(URL_REPRESENTATIVE)
        );
        ...
        return {
            success: false,
            message: error?.message || 'Error create Representative',
        };
    }
}
...
export const deleteRepresentative = async (id: string) => {
    ...
    await mutate((key) => typeof key === 'string' && key.startsWith(URL_REPRESENTATIVE));
    return res;
};