...
export function useGetAllTemplatePositions(batchId?: string) {
    const { data, error, isLoading, isValidating } = useSWR<{ data: ITemplatePosition[] }>(
...
export async function createTemplatePositionBulk(
  ...
  try {
    const res = await request<ApiResponse<any>>(URL_TEMPLATE_POSITION, {
      method: 'POST',
    ...
    return {
      success: false,
      message: error?.message || 'Error bulk create Template Position',
    };
  }
}
...
export async function editTemplatePositionBulk(
  ...
  try {
    ...
    return {
      success: false,
      message: error?.message || 'Error bulk update Template Position',
    };
  }
}
// ===========================
...
export const deleteTemplatePosition = async (id: string) => {
    const res = await request<ApiResponse>(
        `${URL_TEMPLATE_POSITION}/${id}`,
        { method: 'DELETE' }
    );
    await Promise.all([
        mutate(`${URL_TEMPLATE_POSITION}/${id}`),
        mutate((key) => typeof key === 'string' && key.startsWith(URL_TEMPLATE_POSITION)),
    ]);
    return res;
};
...
export async function syncTemplatePositionsBulk(
  ...
  try {
    ...
    return {
      success: false,
      message: error?.message || 'Error sync Template Position',
    };
  }
}